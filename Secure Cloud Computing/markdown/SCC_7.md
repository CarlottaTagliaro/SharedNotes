...
Multiple parties want to compute a program over joint data. We have two clients, A and B, and they both have sensitive data they want to outsource, hence they encrypt those and outsource to the SP. The SP can evaluated the program over the joint inputs from the different parties.

In contrast to FE, we have an interactive setting. The SP can interact with the parties and later perform the computation. 

It is important to distinguish between an online vs offline phase:
...
The initial setting is the same: client A encrypts data before outsourcing to the SP. In contrast with FE, the Cloud SP can perform an operation on his own without the need of a function token and then return the **encrypted** result back to A (no leakage to the CSP). 

We can imagine HE like a “normal” Public Key Encryption scheme:
...
But we need to add a function for computation over ciphertext:

- $Eval (P, c _1 , ... c_ n )$: Evaluate program $P$ over ciphertexts $c_ 1$ to $c_ n$ as program inputs. Output ciphertext $c′$ which is the ciphertext of the program evaluated over the input ciphertexts.
...
- **Somewhat HE** (SWHE): arbitrary number of one operation, limited in other operation, i.e. polynomial evaluation of a limited degree and binary circuit of a limited depth; Example: Boneh-Goh-Nissim [BGN05];
...
This is the most simple HE scheme. Recall that a cyclic group $G$ of order $n$ has the following properties:

- It has $n$ different elements
- The group is equipped with a group operation $⋅$
- One neutral element $1$
- Each element $a$ has an inverse element $a ^{−1}$ with $a ⋅ a ^{−1} = 1$
- There exists at least one element $g ∈ G$ that generates the group, i.e. $\{g_ i : 1 ≤ i ≤ n \}= G$
...
- $Gen (1 ^λ) $: Select generator $g ∈ G$ of order $q$, pick random value $x$ and compute $h = g ^x $. Output $pk = (G, g, q, h)$ and $sk = x $. Here it holds the Discrete Logarithm assumption hence, $x$ cannot be computed given only $h$ and $g$. 
...
1. $G$ and $G _T$ are two multiplicative cyclic groups of finite order $n$
2. There exists a bilinear map $e: G × G → G _T $: for all $u, v ∈ G$ and $a, b ∈ \Z$, it holds $e (u^ a , v ^b) = e (u, v )^{ab}$ for the bilinearity property.
...
- $Gen (1 ^λ) $: Pick random primes $p$, $q$, comùpute $n = p ⋅ q$. Pick two random generators $g, u ∈ G$ and set $h = u^ q$. Output $pk = (n, G, G_ T , e, g, h)$ and $sk = p$; Mind that $h$ generates a subgroup given that $h^p=g^n=1$
...
For decryption we need to calculate the Dlog and, as we learned from ElGamal, this is hard to do. To efficiently compute decryption what we can do is limit the plaintext space so that we can "brute-force" it and learn the decryption.
...
- $S-KeyGen(1 ^λ )$: Sample odd random integer in a given range $p ← [2^{ η−1} , 2 ^η )$ and use it as secret key;

- $S-Enc(p, m)$: Sample random integer $q ← [0, 2 ^γ /p]$ (another domain) and random integer $r ← [−2^ ρ, 2^ρ ] < \frac p 2$ (so again, another domain). Output $c = pq + 2r + m$. 
...
- $S-Dec(p, c)$: Output $m ′ = (c \mod p) \mod 2$. Reducing by modulo $p$ we get a value between $[0,p)$ which is $2r+m$ (remind that $2r<p$ from previous construction, otherwise the scheme won't work). Then by reducing modulo $2$ (removing the noise $2r$), we get a value between $[0,1]$. 
...
Let's see how can we move to **asymmetric** encryption in a quite easy way. What is to be observed is that known values can be added to ciphertexts: $c + a = pq + 2r + m + a$ which is the same as $S−Dec (p, c + a) = m ⊕ a$. 

As for generating a public key, during key generation, we calculate a set of symmetrically encrypted zeroes $X$ as public key (they'll have a different encryption given the randomness introduced). A subset of these zeroes is chosen during encryption and added using the previously seen properties:
$$
Enc (pk, m) = S−Enc (p, 0) + S−Enc (p, 0) + ... + S−Enc (p, 0) + m
$$
...
- $Gen(1 ^λ )$: Sample odd random integer in a given range $p ← [2^{ η−1} , 2 ^η )$ and sample $X= \{S-Enc(p,0)\}$; Output $sk = p$, $pk = X$;
